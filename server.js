...
io.on("connection", (socket) => {
  ...
  socket.on("joinAdminChat", (shopId) => {
    if (shopId === "admin") {
      ...
      console.log(`✅ Admin socket ${socket.id} joined dashboard`);
    } else {
      // Shop joining their admin chat room
      socket.join(`admin-chat-${shopId}`);
      socket.shopId = shopId;
      console.log(`✅ Shop socket ${socket.id} joined admin chat for shop: ${shopId}`);
    }
  });
  ...
  socket.on("adminMessage", (data) => {
    ...
    console.log("📤 Admin message via socket:", data);
